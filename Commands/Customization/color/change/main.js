...
exports.run = function (message, client) {
    fs.readFile('./data.json', 'utf8', function readFileCallback (err, data) { // Read file
        ...
        if (obj.users.some(a=>message.author.id.includes(a.id))) { // Check if id matches author
        ...
        } else { // Check for last loop
            obj.users.pushQ({name: message.author.username, id: message.author.id, color: parseInt(message.content[3], 16)}); // Create new user
            functions.write(message, 'sucess', 'Your color is now set to ' + parseInt(message.content[3], 16), obj);
        }
